...
class AndroidWidgets {
  ...
  registerBackgroundHandler(handler) {
    if (_reactNative.Platform.OS !== "android") return;
    _reactNative.AppRegistry.registerHeadlessTask("RNWidgetBackgroundFetch", () => handler);
  }
  async scheduleBackgroundFetch(widgetName, intervalMinutes) {
    if (_reactNative.Platform.OS !== "android") return false;
    try {
      return await RNAndroidWidgets.scheduleBackgroundFetch(widgetName, intervalMinutes);
    } catch (error) {
      console.error("Failed to schedule background fetch:", error);
      return false;
    }
  }
  async cancelBackgroundFetch(widgetName) {
    if (_reactNative.Platform.OS !== "android") return false;
    try {
